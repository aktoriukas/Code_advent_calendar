...
function checkForLoop(i) {
    let loop = pastInstructions.indexOf(i);
    if (loop < 0) {
        return false
    }
    return true
...
function runCode (newinstructionArray) {
    ...
    for (let i = 0; i < newinstructionArray.length; i) {
        ...
        instruction = newinstructionArray[i].split(' ');
        operationIn = instruction[0];
...
for (let i = 0; i < instructionsArray.length; i++) {
    ...
    if (operation == 'nop') {
        newinstructionArray[i] = instructionsArray[i].replace('nop', 'jmp')
        console.log(`after change: ${newinstructionArray[i]}`)

        if (!runCode(newinstructionArray)) {
            console.log('in loop')
        } else {
