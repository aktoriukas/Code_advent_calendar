...
function validHair (hair) {
    ...
    for (let i = 0; i < hair.length; i++) {
        ...
        // if (hair[i].match(/[a-f]/) == true) {
        //     return false
        // }
        if (parseInt(hair[i]) == NaN) {
            return false
        }
    }
    return true
...
for (index in passports) {
    if (passports[index] == '') {
        passportArray.push(tempArray);
        tempArray = [];
        continue
    }
    tempArray.push(passports[index])

}
passportArray.forEach(function (item, index) {
    ...
    let passportObj = {
        ...
        pid: undefined,
        invalid: false,
        invalidPartTwo: false
    }

    item.forEach(function (ite, indx) {

        // console.log(ite)
        let params = ite.split(' ')

        params.forEach(function (parameter, i) {
            let passportData = parameter.split(':')
            passportObj[passportData[0]] = passportData[1]
        })
    })
    for (daig in mustHave) {
        if (passportObj[mustHave[daig]] === undefined) {
            passportObj.invalid = true
            break
        }
    }
    if(passportObj.invalid == false) {
        counter++
        console.log(`${counter} valid:`)
        console.log(passportObj)
        if (!validYear(passportObj.byr)) { 
            passportObj.invalidPartTwo = true 
            console.log('<!><!><!><!><!><!><!><!> ⇧ invalid year ⇧ <!><!><!><!><!><!><!>')
        }
        if(!validIssue(passportObj.iyr)) {
            passportObj.invalidPartTwo = true 
            console.log('<!><!><!><!><!><!><!><!> ⇧ invalid Issue ⇧ <!><!><!><!><!><!><!>')
        }
        if (!validExpireDate(passportObj.eyr)) {
            passportObj.invalidPartTwo = true 
            console.log('<!><!><!><!><!><!><!><!> ⇧ invalid Expire ⇧ <!><!><!><!><!><!><!>')
        }
        if (!validheight(passportObj.hgt)){
            passportObj.invalidPartTwo = true 
            console.log('<!><!><!><!><!><!><!><!> ⇧ invalid Height ⇧ <!><!><!><!><!><!><!>')
        }
        if(!validHair(passportObj.hcl)) {
            passportObj.invalidPartTwo = true 
            console.log(passportObj.hcl)
            console.log('<!><!><!><!><!><!><!><!> ⇧ invalid Hair ⇧ <!><!><!><!><!><!><!>')
        }
        if(!validEye(passportObj.ecl)){
            passportObj.invalidPartTwo = true 
            console.log('<!><!><!><!><!><!><!><!> ⇧ invalid Eye ⇧ <!><!><!><!><!><!><!>')
        }
        if(!validPass(passportObj.pid)){
            passportObj.invalidPartTwo = true 
            console.log('<!><!><!><!><!><!><!><!> ⇧ invalid Pass ⇧ <!><!><!><!><!><!><!>')
        }
        if (passportObj.invalidPartTwo == false) {
            part2Valid++
        }else {
            part2Invalid++
            console.log('INVALID PART 2')
            console.log('---------------------------------')
        }
    }else {
        invalid++
...
console.log(`total: ${passportArray.length}`)
console.log(`total valid: ${counter}`)
console.log(`total invalid: ${invalid}`)
console.log('====================')
console.log(`part 2 valid: ${part2Valid}`)
console.log(`part 2 invalid: ${part2Invalid}`)
