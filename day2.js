...
passwords.forEach(function(item, index) {
    ...
    if(item[1] != '-'){ // if min not sigle digit
        min += item[1]
        max = item[3]
        if(item[4] != '-') { // if max not single digit
            max += item[4]
        }
    }else { // if min sigle digit
        max = item[2]
        if(item[3] != '-') { // if max not single digit
            max += item[3]
        }
    }
    for( let i = 0; i < item.length; i++) {
        if(item[i] != undefined && item[i].toUpperCase() != item[i].toLowerCase()) {
            letter = item[i]
            break
        }
    }
    ...
    letterCounter = parseInt(letterCounter) - 1;
    ...
    stringg = item.split(':');
    stringg = stringg[1];
    stringg = stringg.substring(1);
    if (stringg[min-1] == letter) {
        if (stringg[max-1] != letter) {
            counter++
        }
    } else {
        if (stringg[max-1] == letter) {
    ...
    // console.log(`minimum: ${min} maximum: ${max} letter: ${letter} letterCounter${letterCounter}`)
})
console.log(counter)
